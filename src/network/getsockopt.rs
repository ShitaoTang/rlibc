...
pub extern "C" fn getsockopt(fd: c_int, level: c_int, optname: c_int, optval: *mut c_void, optlen: *mut socklen_t) -> c_int
{
    ...
    let mut r: c_int = socketcall(SYS_getsockopt as c_int, fd as c_long, level as c_long, optname as c_long, optval as c_long, optlen as c_long, 0) as c_int;
    ...
    libc::SOL_SOCKET => { 
        ...
        libc::SO_RCVTIMEO | libc::SO_SNDTIMEO => {
            ...
            if unsafe { *optlen } < size_of::<libc::timeval>() as socklen_t { return __syscall_ret(-EINVAL as u64) as c_int; }
            if optname == libc::SO_RCVTIMEO { optname = SO_RCVTIMEO_OLD; }
            if optname == libc::SO_SNDTIMEO { optname = SO_SNDTIMEO_OLD; }
            r = socketcall(SYS_getsockopt as c_int, fd as c_long, level as c_long, optname as c_long, tv32.as_mut_ptr() as c_long, (size_of::<c_long>() * 2).try_into().unwrap(), 0) as c_int;
        ...
        libc::SO_TIMESTAMP | libc::SO_TIMESTAMPNS => {
            ...
            r = socketcall(SYS_getsockopt as c_int, fd as c_long, level as c_long, optname as c_long, optval as c_long, optlen as c_long, 0) as c_int;
            return __syscall_ret(r as u64) as c_int;
        }
        _ => {}
        }
